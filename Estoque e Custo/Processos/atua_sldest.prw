...
IndRegua("TRC",_cArqSeq,"AH1_CONTRA+AH1_SEQCON",,,"Selecionando Registros...")
dbGoTop()
_cArqSeq:= '\data\AH2'
dbUseArea(.T.,,_cArqSeq,"TRA",.F.,.F.)
IndRegua("TRA",_cArqSeq,"AH2_CONTRA+AH2_SEQCON",,,"Selecionando Registros...")
...
dbSelectArea("AH2")
aStruct := AH2->(dbStruct())
cNomArq := CriaTrab(aStruct)
cIndex  := cNomArq
dbUseArea( .T.,, cNomArq, "TRA", if(.F. .OR. .F., !.F., NIL), .F. )
IndRegua("TRA",cIndex,"AH2_CONTRA+AH2_SEQCON",,,"Selecionando Registros...")
dbSetIndex( cNomArq +OrdBagExt())             
Dbsetorder(1)
...
For _ncc := 1 to FT_FLASTREC()
	...
	TRC->AH1_DTACON := stod(_cdtinicio)
	TRC->AH1_SEQCON := "01"
	TRC->AH1_CARTRE := "1"
	TRC->AH1_SUB_LI := "1"
	TRC->AH1_MOEDA  := "1"
...
For _ncc := 1 to FT_FLASTREC()
	...
	_ctipo     := Substr(cLinha,87,1) // 87 com 1 posicoes

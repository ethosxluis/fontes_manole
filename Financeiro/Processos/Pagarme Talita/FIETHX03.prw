...
User Function FIETHX03(aRetInf)	
	...
	//{'DTOPER','TPOPER','IDOPER','DESCRI','IDTRAN','PARCEL','METPGTO','ENTBRU','SAIBRU','TXOPER','TXANT','TXTOTAL','ENTLIQ','SAILIQ','RECNO'}
	Begin Transaction
		While TRB->(!Eof()) 
			...
			If !Empty(TRB->X1_OK)
				If (TRB->X1_OK == cMarca) .AND. (!TRB->RECNO == 9999999999)
					...
					Else	
						...
						nValtit := TRB->ENTLIQ
						aRetSE1		:= BaixaSE1(SubStr(TRB->DTOPER,0,10),nValtit+nTxTit,nTxTit)
				...
				ElseIf (TRB->X1_OK == cMarca) .AND. (TRB->RECNO == 9999999999) .AND. !EMPTY(TRB->CLIENTE) .AND. !EMPTY(TRB->LOJA)
					If (TRB->X1_TIPO == "RA")
						if GeraNewSE1(TRB->ENTLIQ,.t.,TRB->CLIENTE,TRB->LOJA,TRB->IDTRAN)
							aAdd(aErros,{TRB->DTOPER,TRB->TPOPER,TRB->IDOPER,TRB->DESCRI,TRB->IDTRAN,TRB->PARCEL,TRB->METPGTO,TRB->ENTBRU,TRB->SAIBRU,TRB->TXOPER,TRB->TXANT,TRB->TXTOTAL,TRB->ENTLIQ,TRB->SAILIQ,TRB->RECNO,TRB->CLIENTE,TRB->LOJA})
						Endif	
					EndIf	
		...
		If nTXTotal > 0
			If GeraNewSE2(nTXTotal) //gera SE2 com a taxa cobrada pelo banco
				aAdd(aErros,{TRB->DTOPER,TRB->TPOPER,TRB->IDOPER,TRB->DESCRI,TRB->IDTRAN,TRB->PARCEL,TRB->METPGTO,TRB->ENTBRU,TRB->SAIBRU,TRB->TXOPER,TRB->TXANT,TRB->TXTOTAL,TRB->ENTLIQ,TRB->SAILIQ,TRB->RECNO,TRB->CLIENTE,TRB->LOJA})
			EndIF
		EndIF
/*
...
/*/
Static function  BaixaSE1(cDat,nValTit,nTxOper)
	Local aBaixa 	:= {}
...
@see (links_or_references)
/*/
Static function GeraNewSE1(nValor,lCliente,cCliRA, cLojaRA,cTID)
	Local cCliente	:= alltrim(getmv('FS_PAGCLI',.T.,'093632'))
	Local cLoja		:= alltrim(getmv('FS_PAGLOJA',.T.,'01'))
	Local cBanc 	:= alltrim(getmv('FS_PAGBANC',.T.,''))
	Local cAg   	:= alltrim(getmv('FS_PAGAGEN',.T.,'')) + Space(5 - Len(getmv('FS_PAGAGEN')))
	Local cCONT 	:= alltrim(getmv('FS_PAGCONT',.T.,''))
...
Static function GeraNewSE2(nTXTotal)

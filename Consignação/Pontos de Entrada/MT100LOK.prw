...
±±ºPrograma  ³MT100LOK  ºAutor  ³TOTVS               º Data ³ 08/11/2010  º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±ºDesc.     ³Ponto de entrada na validacao da Nf de Retorno de produto   º±±
±±º          ³em consignacao.                                             º±±
...
Local aArea   := GetArea()
Local lRet    := .T.
Local cAlmox  := GdFieldGet("D1_LOCAL")
Local cTpOper := GdFieldGet("D1_XOPER")// Retorno Simbolico
Local cAlmoxRet := GetMv("MV_XALMOXC")
// Criar parametro para conter o tipo de operacao de entrada
//Criar parametro para almox = 10
                         
If Alltrim(Funname()) == "LOJA920"
	Return(.T.)
EndIf

If cTpOper $ GetMv("MV_XTOPERE") .And. cAlmox # cAlmoxRet
	Aviso(OemToAnsi('MT100LOK'), 'O armazem de retorno simbolico de consignação deve ser o '+cAlmoxRet+', altere o campo Armazém da nota fiscal', {'Ok'})
	lRet:= .F.
EndIf

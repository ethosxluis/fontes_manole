	...
	dbUseArea( .T.,"TOPCONN",TCGENQRY(,,cQuery),"TRB",.F.,.T.)
		...
		IF TRB->( !EOF () )
			...
			If MsgYesNo("O fechamento do mês/ano "+MV_PAR01+"/"+MV_PAR02+" já foi executado, deseja refazer?")
				...
				Processa({|| Limpa_P03() },"Ajustando os dados...","Aguarde.")
			else
			  ruturn()
			endif
...
//MENSAIS
   ...
   //cquery  +=  " D2_PRUNIT/(SELECT COUNT(*) FROM "+retsqlname("SE1")+ " WHERE E1_PREFIXO = D2_SERIE AND E1_NUM = D2_DOC AND E1_CLIENTE = D2_CLIENTE AND E1_LOJA = D2_LOJA AND D_E_L_E_T_ = ' ') AS PRUNIT, "
   //cquery  +=  " D2_PRCVEN/(SELECT COUNT(*)  FROM "+retsqlname("SE1")+ " WHERE E1_PREFIXO = D2_SERIE AND E1_NUM = D2_DOC AND E1_CLIENTE = D2_CLIENTE AND E1_LOJA = D2_LOJA AND D_E_L_E_T_ = ' ') AS PRCVEN, "
...
//   cquery  +=  " and D2_EMISSAO>= '"+dtos(_dtini)+"'" //ALTERADO PARA MES 2 PARA TESTE DE DEMONSTRAção
   ...
   cquery  +=  " AND P04_XTPFAT = '2'" //calculo pelo financeiro
...
//FINANCEIRO
   ...
   cquery :=  " SELECT P04_CONTRA,P04_SEQCON,P04_FORNEC,P04_LOJAFO,P04_PRODUT,P04_PERIOD,D2_QUANT, "
//   cquery  +=  " D2_PRUNIT/(SELECT COUNT(*)  FROM "+retsqlname("SE1")+ " WHERE E1_PREFIXO = D2_SERIE AND E1_NUM = D2_DOC AND E1_CLIENTE = D2_CLIENTE AND E1_LOJA = D2_LOJA AND D_E_L_E_T_ = ' ') AS D2_PRUNIT, "
//   cquery  +=  " D2_PRCVEN/(SELECT COUNT(*)  FROM "+retsqlname("SE1")+ " WHERE E1_PREFIXO = D2_SERIE AND E1_NUM = D2_DOC AND E1_CLIENTE = D2_CLIENTE AND E1_LOJA = D2_LOJA AND D_E_L_E_T_ = ' ') AS D2_PRCVEN, "
   cquery  +=  " D2_PRUNIT, "
   cquery  +=  " D2_PRCVEN, "
   cquery  +=  " D2_DOC,D2_CLIENTE,D2_LOJA, "
...
//   cquery  +=  " and D2_EMISSAO>= '20200501'"
   cquery  +=  " AND E1_SERIE = D2_SERIE "
   cquery  +=  " AND E1_NUM = D2_DOC "
   cquery  +=  " AND E1_CLIENTE = D2_CLIENTE "
   cquery  +=  " AND E1_LOJA = D2_LOJA "
...
//P03_FILIAL+P03_MESANO+P03_CONTRA+P03_SEQCON+P03_FORNEC+P03_LOJAFO+P03_PRODUT +P03_SERIE+P03_DOC+P03_CLIENT+P03_LOJCLI+P03_PARC                                     
...
///////////////////////FATURAMENTO
   		...
   		cquery += " WHERE P04_PRODUT = D2_COD "
   		cquery += " AND P04_PERIOD = '"+_period+"' "
   		cquery += " AND P04_XTPFAT = '1'" //FATURAMENTO
   		cquery += " AND D2_TES IN(SELECT F4_CODIGO FROM  "+RetSQLName("SF4")+ " WHERE F4_TIPOPER = '5' AND D_E_L_E_T_ <> '*') "
   		cquery += " and D2_EMISSAO BETWEEN  '"+dtos(_dtini)+"' AND '"+dtos(_dtfim)+"'"
   		cquery += " AND "+RetSQLName("P04")+ ".D_E_L_E_T_ <> '*' "
   		cquery += " AND "+RetSQLName("SD2")+ ".D_E_L_E_T_ <> '*' "

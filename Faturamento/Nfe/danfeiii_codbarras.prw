// ************************************** //
// PARA PESQUISAR AS CUSTOMIZAÇÕES MANOLE //
// DIGITAR "CUSTOMIZADO" NA PESQUISA      //
// ************************************** //
#INCLUDE "PROTHEUS.CH" 
#INCLUDE "TBICONN.CH"
#INCLUDE "COLORS.CH"
...
//	nPerDesc	:= ((nDesc*100)/nVTotal)
	...
	AADD(aAux, AllTrim(TransForm(((nDesc*100)/nVTotal),TM(((nDesc*100)/nVTotal),TamSX3("D2_DESCON")[1],TamSX3("D2_DESCON")[2]))))
	// CUSTOMIZADO FIM
	...
	//CUSTOMIZADO - EDMAR
	//SubStr(NoChar(oDet[nX]:_Prod:_xProd:TEXT + 'Código de Barras: ' + oDet[nX]:_Prod:_cEan:TEXT,lConverte),1,nMaxDes),;
	//PADRAO: SubStr(NoChar(oDet[nX]:_Prod:_xProd:TEXT,lConverte),1,nMaxDes),;
	aadd(aItens,{;
		SubStr(oDet[nX]:_Prod:_cProd:TEXT,1,nMaxCod),;
		SubStr(oDet[nX]:_Prod:_cEan:TEXT,1,nMaxcEan),;
	...
	If ( oDet[nX]:_Prod:_uTrib:TEXT <> oDet[nX]:_Prod:_uCom:TEXT )
		...
		AADD(aAuxCom, AllTrim(TransForm(nVUnitCom,TM(nVUnitCom,TamSX3("D2_PRCVEN")[1],4))))
		// CUSTOMIZADO FIM
	...
	//ALTERADO
	//cAux     := AllTrim(SubStr(NoChar(oDet[nX]:_Prod:_xProd:TEXT + ' Código de Barras: ' + oDet[nX]:_Prod:_cEan:TEXT,lConverte),(nMaxDes + 1)))
	//cAuxItem := AllTrim(SubStr(oDet[nX]:_Prod:_cProd:TEXT + ' Código de Barras: ' + oDet[nX]:_Prod:_cEan:TEXT,nMaxCod+1))
	
	cAuxItem := AllTrim(SubStr(oDet[nX]:_Prod:_cProd:TEXT,nMaxCod+1))
	cAuxcEan := AllTrim(SubStr(oDet[nX]:_Prod:_cEan:TEXT,nMaxcEan+1))
	cAux     := AllTrim(SubStr(NoChar(oDet[nX]:_Prod:_xProd:TEXT,lConverte),(nMaxDes + 1)))	
	...
	While !Empty(cAux) .Or. !Empty(cAuxItem) .Or. !Empty(cAuxcEan) .Or. !Empty(aAux[1]) .Or. !Empty(aAux[2]) .Or. !Empty(aAux[3]) .Or. !Empty(aAux[4]);
		...
		cAuxcEan    := SubStr(cAuxcEan,nMaxcEan+1)
		aAux[1]     := SubStr(aAux[1], PosQuebrVal(aAux[1]) + 1)
		aAux[2]     := SubStr(aAux[2], PosQuebrVal(aAux[2]) + 1)
		aAux[3]     := SubStr(aAux[3], PosQuebrVal(aAux[3]) + 1)
		aAux[4]     := SubStr(aAux[4], PosQuebrVal(aAux[4]) + 1)
...
*/
// CUSTOMIZADO INICIO
If !EMPTY(SC5->C5_OBJETO) .or. !EMPTY(SC5->C5_NUM)
	if !EMPTY(SC5->C5_OBJETO) .and. !EMPTY(SC5->C5_NUM)
		cAux := "Numero do Rastreio: " + SC5->C5_OBJETO +" Numero do Pedido: " + SC5->C5_NUM
	elseif !EMPTY(SC5->C5_OBJETO) .and. EMPTY(SC5->C5_NUM)
		cAux := "Numero do Rastreio: " + SC5->C5_OBJETO
	elseif EMPTY(SC5->C5_OBJETO) .and. !EMPTY(SC5->C5_NUM)
		cAux := " Numero do Pedido: " + SC5->C5_NUM
	endif
	...
	If SA1->(MsSeek(xFilial("SA1")+ SC5->C5_CLIENTE + SC5->C5_LOJACLI,.T.))
		If !Empty(SA1->A1_ENDENT)
			cAux := "Endereco de Entrega: " + Alltrim(SA1->A1_ENDENT)
		EndIf 
	EndIf
...
For nX :=1 To nLenItens
	...
	nY++
	aadd(Atail(aAux)[nY],NoChar(aItens[nX][03],lConverte))
	nY++
...
If aDest[07] == "MG"
	aTamCol[1] -= 15	//"COD. PROD",;
	aTamCol[2] -= 10	//"COD BARRAS",;
	aTamCol[3] -= 5		//"DESCR PROD",;
	aTamCol[4] += 0		//"NCM/SH",;
	aTamCol[5] += 0		//IIf( cMVCODREG == "1", "CSOSN","CST" ),;
	aTamCol[6] += 0		//"CFOP",;
	aTamCol[7] += 0		//"UN",;
	aTamCol[8] += 0		//"QUANT.",;
	aTamCol[9] += 0		//"V.UNITARIO",;
	aTamCol[10] += 0	//"VLR TOTAL",;
	aTamCol[11] += 0	//	"VLR DESC",;
	aTamCol[12] += 0	//"V.UNI LIQ",;
	aTamCol[13] += 0	//	"TOTAL LIQ",;
	aTamCol[14] += 0	//	"BC.ICMS",;
	aTamCol[15] += 0	//	"BC.ICMS ST",;
	aTamCol[16] += 0	//	"VLR ICMS",;
	aTamCol[17] += 0	//	"VLR ICMS ST",;
	aTamCol[18] += 0	//	"VALOR IPI",;
	aTamCol[19] += 0	//	"ICMS",;
	aTamCol[20] += 0	//	"IPI";
Else
	aTamCol[1] -= 15	//"COD. PROD",;
	aTamCol[2] -= 35	//"COD BARRAS",;
	aTamCol[3] -= 10	//"DESCR PROD",;
	aTamCol[4] += 0		//"NCM/SH",;
	aTamCol[5] += 0		//IIf( cMVCODREG == "1", "CSOSN","CST" ),;
	aTamCol[6] += 0		//"CFOP",;
	aTamCol[7] += 0		//"UN",;
...
If nFolha == 1
	...
	//EDMAR CUSTOMIZADO
	AADD(aColProd, {nAux, nAux + aTamCol[2]}) //"COD BARRAS"
	nAux += aTamCol[2]
	...
	nAux += aTamCol[10]
	//CUSTOMIZADO INICIO
	AADD(aColProd, {nAux, nAux + aTamCol[11]}) // "PER DESC"
 	nAux += aTamCol[11]
	...
	oDanfe:Say(nLine+462, nAux + 2,"COD BARRAS", oFont08N:oFont)
	nAux += aTamCol[2]

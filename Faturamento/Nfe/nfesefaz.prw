...
If cTipo == "1"
	...
	For nCountNF := 1 To Len(aCupRefLoj)
		...
		//						If MsSeek(xFilial("SD2")+(cAliasSD2)->D2_NFORI+(cAliasSD2)->D2_SERIORI+(cAliasSD2)->D2_CLIENTE+(cAliasSD2)->D2_LOJA+(cAliasSD2)->D2_COD+(cAliasSD2)->D2_ITEMORI)
						...
						//Customizado Manole
						//14/11/2019 - Edmar Mendes do Prado
						//Retirado para não ser impresso o código do cliente no DANFE
						/* 
...
If cVeramb >= "3.10"
	...
	cString += '<dhEmi>'+ConvType(aNota[03])+"T"+Iif(Len(AllTrim(aNota[06])) > 5,ConvType(aNota[06]),IIF(!EMPTY(aNota[06]),ConvType(aNota[06]),SUBSTR(TIME(),1,5))+":00")+'</dhEmi>'
	cString += NfeTag('<dhSaiEnt>',Iif(lDSaiEnt,"",ConvType(aNota[03])+"T"+Iif(Len(AllTrim(aNota[06])) > 5,ConvType(aNota[06]),IIF(!EMPTY(aNota[06]),ConvType(aNota[06]),SUBSTR(TIME(),1,5))+":00")))
Else
	cString += '<dEmi>'+ConvType(aNota[03])+'</dEmi>'
	cString += NfeTag('<dSaiEnt>',Iif(lDSaiEnt, "", ConvType(aNota[03])))
	If !lDSaiEnt .And. !Empty(aNota[06])
...
Static Function NfeDest(aDest,cVerAmb,aTransp,aCST,lBrinde,cMunDest)
	...
	//Customizado - Inicio
	//06/02/2019 - Edmar Mendes do Prado
	Local cMailSC5		:= ""
	...
	If ! Empty(SC5->C5_XEMAIL)
		cMailSC5 := ";" + Alltrim(SC5->C5_XEMAIL)
	EndIf
	...
	if !lBrinde
		cString += NfeTag('<EMAIL>',AllTrim(aDest[16])+cMailTrans+cMailSC5)
	endif
...
If lNoImp2UM			//Para parametro MV_NIMP2UM = .T.
